...
if (isset($_GET['Tipoticket'])) {
  ...
  if ($Tipoticket == 1) {
    $sql = "SELECT * FROM tbl_calendario WHERE estado_bitcora='Solucionado'";
  }

  if ($Tipoticket == 2) {
    $sql = "SELECT * FROM tbl_calendario WHERE estado_bitcora='Pendiente'";
  }

  if ($Tipoticket == 3) {
    $sql = "SELECT * FROM tbl_calendario WHERE estado_bitcora='Rechazado'";
  }

  if ($Tipoticket == 4) {
    $sql = "SELECT * FROM tbl_calendario WHERE estado_bitcora='En proceso'";
  }
} else {
  $sql = "SELECT * FROM tbl_calendario";
}
...
class PDF extends FPDF
{
  ...
  function Header()
  {
    ...
    $this->Cell(89, 8, 'Reportes de HelpDesk - Total de Eventos Creados', 0, 1);
    $this->SetY(35);
    $this->SetX(14);
...
while ($tik = mysqli_fetch_array($query)) {
  ...
  $pdf->Cell(180, 8, utf8_decode("Id : " . $tik['id'] . "."),  'TLR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Cell(180, 8, utf8_decode("Evento: " . $tik['evento'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Cell(180, 8, utf8_decode("Fecha Inicio: " . $tik['fecha_inicio'] . "                                                                                               Fecha Final: " . $tik['fecha_fin']),  'LR', 1, 'L', 1);
  ...
  $pdf->SetX(15);
  $pdf->MultiCell(180, 8, utf8_decode("Nombre Usuario: " . $tik['nombre_usuario'] . "."), 'LR', 1, 'L', 1);


  $pdf->SetX(15);
  $pdf->MultiCell(180, 8, utf8_decode("Correo: " . $tik['correo'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->MultiCell(180, 8, utf8_decode("Departamento: " . $tik['departamento_ticket'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Área Solicitud: " . $tik['area_solicitud'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Regional: " . $tik['regional_ticket'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Técnicos Asignados: " . $tik['tecnicos_ticket'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Problema Presentado: " . $tik['problema_presentado'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Serie: " . $tik['serie'] . "."), 'LR', 1, 'L', 1);

  $pdf->SetX(15);
  $pdf->Multicell(180, 8, utf8_decode("Estado: " . $tik['estado_bitacora'] . "."), 'BLR', 1, 'L', 1);
