...
$cont = 1;
$salida .= "<table>";
$salida .= "<thead> <th>#</th> <th>Fecha</th> <th>Serie</th> <th>Estado del ticket</th> <th>Usuario creador</th> <th>Email</th> <th>Departamento</th> <th>Problema Presentado</th> <th>Asignado a</th></thead>";
while ($ticket = mysqli_fetch_array($query)) {
    $salida .= "<tr> <td>" . $cont . "</td> <td>" . $ticket['fecha'] . "</td> <td>" . $ticket['serie'] . "</td> <td>" . utf8_decode($ticket['estado_ticket']) . "</td> <td>" . utf8_decode($ticket['nombre_usuario']) . "</td> <td>" . $ticket['email'] .
        "</td> <td>" . utf8_decode($ticket['id_dept']) . "</td> <td>" . utf8_decode($ticket['asunto']) . "</td> <td>" . utf8_decode($ticket['asignar_ticket']) . "</td> </tr>";
    $cont++;
}
$salida .= "</table>";
